package streaming.batchopt.simulator;
...
public class StreamingSimulatorBatchOpt extends StreamingSimulator<SingleFSDetection> {
	...
	public void initializationForEachDevice(int deviceId, String strInDevice){
		...
		singleFSDetection.initialization(strInDevice.split(","), batchSize);
		singleFSDetection.addCurrentFreqPatternToHistroy();
		this.singleSequenceSimulator.put(deviceId, singleFSDetection);
	}

	public static void main(String[] args) {
		System.out.println("New Version of Streaming Algorithm with batch OPT");
		...
		try {
			...
			StreamingSimulatorBatchOpt globalFS = new StreamingSimulatorBatchOpt(parameterSpace, new RuntimeStatistics());
