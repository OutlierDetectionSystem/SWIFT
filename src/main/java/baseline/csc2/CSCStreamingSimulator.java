 ...
 */
public class CSCStreamingSimulator extends StreamingSimulator<SingleFSDetection> {

	public CSCStreamingSimulator(ParameterSpace parameterSpace,
...
//		singleFSDetection.addCurrentFreqPatternToHistroy();
	...
	public static void main(String[] args) {
		...
		options.addOption("e", true, "event gap");
		...
		try {
			...
			if(cmd.hasOption("e")){
				...
				int eventGap = Integer.parseInt(cmd.getOptionValue("e"));
				parameterSpace.itemgap = eventGap;
			}
			int printCount = 10;
			if(cmd.hasOption("p")){
			...
			CSCStreamingSimulator globalFS = new CSCStreamingSimulator(parameterSpace, runtimeStatisticsSeqKrimp);
//			HashSet<String> frequentEvents = FileUtile.getFrequentEvents(parameterSpace.inputPath);
